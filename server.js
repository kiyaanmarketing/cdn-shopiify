...
app.post('/api/track-user', async (req, res) => {
  ...
  try {
    ...
    // Sabhi placeholders ko unique_id se replace karo
    // Handles: {replace_it}, {1}, {2}, {21} — koi bhi {anything}
    const finalUrl = affiliateUrl.replace(/\{[^}]*\}/g, unique_id);
    res.json({ success: true, affiliate_url: finalUrl });
  } catch (error) {
    console.error("Error in /api/track-user:", error.message);
